...
uint8_t W25Q64_WriteEnable(void)//写使能，当你需要修改FLASH内存的时候需要打开 而且只能从1改0 不能从0改1
{   uint8_t tx=0x06;
...
uint8_t W25Q64_ReadStatus1(uint8_t *status_reg)//读取w25q64当前状态
{
...
uint8_t W25Q64_ReadJEDEC_ID(uint8_t *manufacturer,//读取设备ID
                            uint8_t *memory_type,
                            uint8_t *capacity)
...
uint8_t W25Q64_WaitBusy(void) //等待w25q64忙完
{   uint8_t status_reg;
	do{
...
uint8_t W25Q64_SectorErase(uint32_t address) //扇区擦除函数 需要先打开写使能并且需要等擦除完成
...
uint8_t W25Q64_ReadData(uint32_t address,uint8_t *data,uint16_t length) //读取数据函数
...
uint8_t W25Q64_PageProgram(uint32_t address,uint8_t *data,uint16_t length) //页编程函数 需要先打开写使能 再等待编程完成
